...
<p align="center">
  <img src="https://img.shields.io/badge/版本-1.2.8.0-blue.svg" alt="版本">
...
项目使用 `public/app.config.json` 进行配置，支持运行时动态加载和修改，刷新页面即可生效。
...
- `osm`：OpenStreetMap，国际标准地图，国内几乎无法访问
- `tianditu-vec`：天地图-矢量（需要 Token）
- `tianditu-img`：天地图-影像（需要 Token）
- `tianditu-ter`：天地图-地形（需要 Token）

### 完整配置说明

详细的配置项说明请查看 [配置文档](./docs/CONFIGURATION.md)。
...
测试脚本会依次执行：
1. **代码质量检查**：ESLint + TypeScript 类型检查（并行）
2. **测试执行**：运行所有测试并生成覆盖率报告
3. **项目构建**：验证生产构建是否成功
